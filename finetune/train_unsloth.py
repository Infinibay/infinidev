#!/usr/bin/env python3
"""Fine-tune with Unsloth — single GPU, 4-bit QLoRA.

Fastest option when the model fits in one GPU. For Qwen3-32B in 4-bit (~17GB),
it fits on an A5000 (24GB) with seq_len ≤ 2048 and gradient checkpointing.
For longer sequences, use train.py with DeepSpeed ZeRO-3 instead.
...
OUTPUT_DIR = BASE_DIR / "output" / "model_unsloth"


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default="unsloth/Qwen2.5-Coder-14B-Instruct-bnb-4bit")
    ...
    # Disable fix_untrained_tokens — causes CUDA assert on Qwen vocab mismatch
    import unsloth_zoo.tokenizer_utils as _tu
    _tu.fix_untrained_tokens = lambda *a, **kw: None

    # ── 1. Load model on single GPU ─────────────────────────────────────
    ...
    # ── 5. Train only on assistant responses ─────────────────────────────
    ...
    print(f"\nTo import to Ollama:")
    print(f"  ollama create infinidev-coder -f {gguf_dir}/Modelfile")
