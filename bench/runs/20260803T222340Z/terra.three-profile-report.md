...
Unanimous questions: 1; divergent questions: 9.
- **raw-prior**: 10 preference choices; 0 errors; median latency 2.411s.
- **fast-autonomy**: 10 preference choices; 0 errors; median latency 1.945s.
- **quality-control**: 10 preference choices; 0 errors; median latency 2.805s.
...
Observed responses:
- **raw-prior** selected **C** — Use the locally dominant name in a small draft diff and invite correction before broader work.; profile: `none`; raw: `{"answer":"C"}`
- **fast-autonomy** selected **D** — Choose the convention used by the nearest analogous helper, implement it, and disclose the assumption.; profile: `fast-autonomy`; raw: `{"answer":"D"}`
...
Observed responses:
- **raw-prior** selected **A** — Propose local-convention defaults for all four and ask for a single confirmation or corrections.; profile: `none`; raw: `{"answer":"A"}`
...
- **quality-control** selected **D** — Ask one question at a time and use each answer to frame the next interaction.; profile: `quality-control`; raw: `{"answer":"D"}`
Observed pattern: models diverged.
...
- **quality-control** selected **C** — Gather input from all affected stakeholders before recommending.; profile: `quality-control`; raw: `{"answer":"C"}`
Observed pattern: models diverged.
...
- **fast-autonomy** selected **D** — Review at one week, with automatic escalation on threshold breach.; profile: `fast-autonomy`; raw: `{"answer":"D"}`
...
Observed responses:
- **raw-prior** selected **B** — Apply the nearest established visual convention, preserve behavior, and show the resulting diff.; profile: `none`; raw: `{"answer":"B"}`
- **fast-autonomy** selected **B** — Apply the nearest established visual convention, preserve behavior, and show the resulting diff.; profile: `fast-autonomy`; raw: `{"answer":"B"}`
...
- **quality-control** selected **B** — Name confidence and both unresolved details.; profile: `quality-control`; raw: `{"answer":"B"}`
Observed pattern: models diverged.
...
- **quality-control** selected **C** — Explain first, implement, then connect changes to concepts.; profile: `quality-control`; raw: `{"answer":"C"}`
Observed pattern: models diverged.
