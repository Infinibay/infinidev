...
class TestCreateFile:
    ...
    def test_create_new_file(self, bound_tool, workspace_dir):
        ...
        path = str(workspace_dir / "new_file.py")
        result = tool._run(path=path, content="print('hello')\n")
    ...
    def test_fail_if_exists(self, bound_tool, workspace_dir):
        ...
        tool = bound_tool(CreateFileTool)
        path = str(workspace_dir / "sample.txt")  # already exists from fixture
        result = tool._run(path=path, content="overwrite")
        data = json.loads(result)
    ...
    def test_creates_parent_directories(self, bound_tool, workspace_dir):
        ...
        tool = bound_tool(CreateFileTool)
        path = str(workspace_dir / "deep" / "nested" / "dir" / "file.txt")
        result = tool._run(path=path, content="nested content\n")
        data = json.loads(result)
        assert data["action"] == "created"
        assert os.path.exists(path)
    ...
    def test_audit_trail(self, bound_tool, workspace_dir, temp_db):
        ...
        tool._run(path=path, content="audit me\n")
    ...
    def test_content_size_limit(self, bound_tool, workspace_dir):
        ...
        try:
            path = str(workspace_dir / "big.txt")
            result = tool._run(path=path, content="x" * 200)
            data = json.loads(result)
            assert "error" in data
    ...
    def test_sandbox_blocking(self, bound_tool, workspace_dir, sandbox_enabled):
        ...
        with patch.object(tool, "_is_pod_mode", return_value=False):
            result = tool._run(path="/tmp/outside_sandbox.txt", content="blocked")
        data = json.loads(result)
        assert "error" in data
