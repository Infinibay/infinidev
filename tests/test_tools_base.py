...
class TestBaseToolStandalone:
    """The tool base is a self-contained pydantic model — no crewai dependency."""

    def test_mro_has_no_crewai(self):
        """InfinibayBaseTool must not derive from anything in the crewai package."""
        assert all("crewai" not in str(c) for c in InfinibayBaseTool.__mro__)

    def test_description_is_not_mutated(self):
        """A tool's LLM-facing description is its declared text, verbatim.

        crewai used to rewrite ``description`` into a verbose "Tool Name: …\\n
        Tool Arguments: …" blob. Removing crewai means the model now sees the
        clean declared description — lock that in.
        """
        ...
        assert "Tool Arguments" not in schema["function"]["description"]
        # Parameters must still be derived from the declared args_schema.
        assert "file_path" in schema["function"]["parameters"]["properties"]

    def test_args_schema_autogenerated_from_run_signature(self):
        """A tool that declares no args_schema gets one built from _run()."""
        tool = _DummyTool()  # no args_schema; _run(self, message: str = "hello")
        assert "message" in tool.args_schema.model_fields
